...
_RAW_FEATURE_CACHE_PATH = os.path.join(data_folder, "raw_feature_cache.pt")
...
def get_node_feature():
    ...
    if (not FORCE_REBUILD_FEATURE_CACHE) and os.path.exists(_RAW_FEATURE_CACHE_PATH):
        raw_feature = torch.load(_RAW_FEATURE_CACHE_PATH)
        return raw_feature

    raw_feature = get_networks(subject_IDs, kind='correlation', atlas_name=atlas)
    os.makedirs(os.path.dirname(_RAW_FEATURE_CACHE_PATH), exist_ok=True)
    torch.save(raw_feature, _RAW_FEATURE_CACHE_PATH)
    return raw_feature
