...
class dataloader():
    ...
    def load_data(self):
        ...
        def _load_time_series(sub_id):
            fl = os.path.join(opt.data_path, sub_id,
                              sub_id + "_" + "ho" + "_" + "time_series" + ".mat")
            return sio.loadmat(fl)["time_series"]

        def _load_dynamic_fc(sub_id):
            fl = os.path.join(opt.data_path, sub_id,
                              sub_id + "_" + "ho" + "_" + "dynamic_fc" + ".mat")
            return sio.loadmat(fl)["dynamic_fc"]

        with concurrent.futures.ThreadPoolExecutor() as executor:
            time_series = list(executor.map(_load_time_series, subject_IDs))
            dynamic_fc = list(executor.map(_load_dynamic_fc, subject_IDs))
        return self.raw_features, self.y, phonetic_data, phonetic_score, time_series, dynamic_fc

    def data_split(self, n_folds):
        """Create cross-validation splits for site-wise leave-one-site-out CV.

        This function merges sub-sites into single logical sites, then selects
        the three largest sites (by subject count) and performs leave-site-out
        cross-validation on them. For each selected site in turn, that site is
        used as the test set and all remaining sites are used for training.
        """

        subject_list = get_ids()
        site_dict = get_subject_score(subject_list, score='SITE_ID')

        # Build an array of raw site labels aligned with subject indices
        raw_site_labels = np.array([site_dict[sid] for sid in subject_list])

        # Merge sub-sites into unified logical sites, e.g. UM_1/UM_2 -> UM,
        # UCLA_1/UCLA_2 -> UCLA. Other sites remain unchanged.
        merged_site_labels = []
        for s in raw_site_labels:
            if s in ['UM_1', 'UM_2']:
                merged_site_labels.append('UM')
            elif s in ['UCLA_1', 'UCLA_2']:
                merged_site_labels.append('UCLA')
            else:
                merged_site_labels.append(s)
        merged_site_labels = np.array(merged_site_labels)

        # Fixed target sites after merging sub-sites.
        # We assume that NYU, UM and UCLA are the three largest sites
        # in the merged setting.
        target_sites = ['NYU', 'UM', 'UCLA']

        cv_splits = []
        for s in target_sites:
            test_idx = np.where(merged_site_labels == s)[0]
            if test_idx.size == 0:
                # If a target site is not present in the current dataset,
                # simply skip it.
                continue
            train_idx = np.where(merged_site_labels != s)[0]
    ...
    def build_similarity_matrix(self, X, K):
        ...
        N = len(sigmas)
        weighted_kernel_matrix = np.zeros((n_samples, n_samples))

        for sigma in sigmas:
            ind = np.zeros((n_samples, K), dtype=int)
            val = np.zeros((n_samples, K))
            for i in range(n_samples):
                ind[i], val[i] = t.get_nns_by_item(i, K, include_distances=True)
            kernel_matrix = np.exp(-(val ** 2) / (2 * sigma ** 2))
            weighted_kernel_matrix += kernel_matrix / N

        return weighted_kernel_matrix
    ...
    def create_type_mask(self, embeddings, phonetic_score):
        ...
        # build_similarity_matrix = self.build_similarity_matrix(site_embeddings_torch, 10)
    ...
    if _PHENOTYPE_CACHE is None:
        ...
        with open(phenotype) as csv_file:
            ...
            for row in reader:
                sub_id = row['SUB_ID']
                cache[sub_id] = row
        _PHENOTYPE_CACHE = cache

    scores_dict = {}
    for sub_id in subject_list:
        row = _PHENOTYPE_CACHE.get(sub_id)
        if row is not None:
            scores_dict[sub_id] = row[score]
    return scores_dict


def get_ids(num_subjects=None):
    global _SUBJECT_IDS_CACHE
    if _SUBJECT_IDS_CACHE is None:
        _SUBJECT_IDS_CACHE = np.genfromtxt(opt.subject_IDs_path, dtype=str)
    if num_subjects is not None:
        return _SUBJECT_IDS_CACHE[:num_subjects]
    return _SUBJECT_IDS_CACHE
...
def build_similarity_matrix(X, K):
    n_samples, n_features = X.shape
    # 使用Annoy构建索引
    t = AnnoyIndex(n_features, 'euclidean')
    for i in range(n_samples):
    ...
    sigmas = [1.0, 1.25, 1.5, 1.75, 2.0]
    neighbor_range = np.arange(K / 2.0, K * 1.5, K / 10.0)
    N = len(sigmas) * len(neighbor_range)
    weighted_kernel_matrix = np.zeros((n_samples, n_samples))
    ...
    for sigma in sigmas:
        for k in neighbor_range:
            ...
            for i in range(n_samples):
                ind[i], val[i] = t.get_nns_by_item(i, k, include_distances=True)
            kernel_matrix = np.exp(-(val ** 2) / (2 * sigma ** 2))
            temp_matrix = np.zeros((n_samples, n_samples))
            for i in range(n_samples):
                temp_matrix[i, ind[i]] = kernel_matrix[i]
            weighted_kernel_matrix += temp_matrix / N

    return weighted_kernel_matrix
def create_affinity_graph_from_embeddings(pd_dict, peoples_feature):
    n_components = 100
    pca = PCA(n_components=n_components)
    reduced_embeddings = pca.fit_transform(peoples_feature.squeeze(1).detach().cpu())

    similarity_matrix = build_similarity_matrix(reduced_embeddings, 10)
    np.fill_diagonal(similarity_matrix, 0)
    num_nodes = 871
    graph = np.zeros((num_nodes, num_nodes))
    similarity_matrix_index = np.argsort(-similarity_matrix, axis=1)[:, :5]

    # for k in range(num_nodes):
    #     for j in range(k + 1, num_nodes):
    #         if similarity_matrix[k][j]>0.1:
    #             graph[k, j] += 1
    #             graph[j, k] += 1

    for k in range(num_nodes):
        for j in range(1):
            graph[k, similarity_matrix_index[k][j]] += 1
            graph[similarity_matrix_index[k][j], k] += 1

    # print('similarity_matrix:',similarity_matrix[similarity_matrix>0.8].size)
...
def get_static_affinity_adj(pd_dict, peoples_feature):
    ...
    # pd_affinity2 = create_affinity_graph_from_embeddings( pd_dict, peoples_feature)
