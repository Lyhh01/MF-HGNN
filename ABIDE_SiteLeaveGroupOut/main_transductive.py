...
if __name__ == '__main__':
    ...
    # Initializes the data partition for cross-validation.
    # With leave-site-out CV, the number of folds equals the number of unique sites.
    cv_splits = dl.data_split(opt.n_folds)
    n_folds = len(cv_splits)

    # Create training sets, validation sets, and test sets based on CV splits.
    train_index, val_index, test_index = [], [], []
    for i in range(len(cv_splits)):
        test_index.append(cv_splits[i][1])
        # Use the next fold as validation; wrap around by total number of folds
        next_i = (1 + i) % n_folds
        val_index.append(cv_splits[next_i][1])
        set = np.setdiff1d(cv_splits[i][0], cv_splits[next_i][1])
