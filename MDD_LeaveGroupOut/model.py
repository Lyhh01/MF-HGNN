...
def biased_random_walk(adj_matrix, node_scores, x, node_features_all3, perm, max_path_length=10):
    ...
    all_visited = torch.zeros(112, dtype=torch.bool, device=opt.device)
...
class Brain_connectomic_graph(torch.nn.Module):
    ...
    def _setup(self):
        ...
        self.socre_gcn = ChebConv(112, 9, K=3, normalization='sym')
        ...
        self.transformer_convs1=(TransformerConv(in_channels=112, out_channels=64, heads=1))
        ...
        self.topkpool = TopKPooling(in_channels=112, ratio=0.7)
        self.socre_gcn2 = ChebConv(112, int(78), K=3, normalization='sym')

        self.graphnn_left = nn.Sequential(nn.Linear(112, 7, bias=False), nn.ReLU(), nn.Linear(7, 20 * 112))
        self.myginconv_left = MyGINConvWithMean(112, 20, self.graphnn_left, normalize=False)

        self.graphnn_right = nn.Sequential(nn.Linear(112, 7, bias=False), nn.ReLU(), nn.Linear(7, 20 * 112))
        self.myginconv_right = MyGINConvWithMean(112, 20, self.graphnn_right, normalize=False)

        self.graphnn_whole_brain_time = nn.Sequential(nn.Linear(112, 7, bias=False), nn.ReLU(), nn.Linear(7, 20 * 112))
        self.myginconv_whole_brain_time = MyGINConvWithMean(112, 20, self.graphnn_whole_brain_time, normalize=False)

        self.graphnn_whole_brain_space = nn.Sequential(nn.Linear(112, 7, bias=False), nn.ReLU(), nn.Linear(7, 20 * 112))
        self.myginconv_whole_brain_space = MyGINConvWithMean(112, 20, self.graphnn_whole_brain_space, normalize=False)

        self.graphnn_all = nn.Sequential(nn.Linear(112, 7, bias=False), nn.ReLU(), nn.Linear(7, 20 * 65))
        self.myginconv_all = MyGINConvWithMean(65, 20, self.graphnn_all, normalize=False)

        self.graphnn_all2 = nn.Sequential(nn.Linear(112, 7, bias=False), nn.ReLU(), nn.Linear(7, 20 * 20))
        self.myginconv_all2 = MyGINConvWithMean(20, 20, self.graphnn_all2, normalize=False)

        self.graphnn_all3 = nn.Sequential(nn.Linear(112, 7, bias=False), nn.ReLU(), nn.Linear(7, 20 * 20))
        ...
        self.conv_whole_brain_logit_k = nn.Linear(25, 25, bias=False)
        self.conv_whole_brain_logit_q = nn.Linear(40, 25, bias=False)
        self.conv_whole_brain_logit_v = nn.Linear(40, 40, bias=False)

        self.conv_whole_brain_logit_k2 = nn.Linear(40, 25, bias=False)
        self.conv_whole_brain_logit_q2 = nn.Linear(25, 25, bias=False)
        self.conv_whole_brain_logit_v2 = nn.Linear(25, 25, bias=False)
...
#############################################
        ...
        adj_matrix = torch.sparse_coo_tensor(edges, edge_attr, (112, 112))
...
class HPG(nn.Module):
    def __init__(self):
        ...
        self.convs1.append(TransformerConv(in_channels=2020,out_channels=20,heads=1))
        self.convs2.append(TransformerConv(in_channels=2020, out_channels=20,heads=1))
        self.convs3.append(TransformerConv(in_channels=2020,out_channels=20,heads=1))
        self.convs4.append(TransformerConv(in_channels=2020, out_channels=20,heads=1))
...
class fc_hgnn(torch.nn.Module):
    ...
    def forward(self, graphs, time_series, dynamic_fc):
        ...
        if self._hpg_inputs is None:
            same_index, diff_index, edge_index, same_index2, diff_index2 = self.dl.get_inputs(self.nonimg, embeddings.detach(), self.phonetic_score, graphs)
            ...
            self._hpg_inputs = (same_index, diff_index, edge_index, same_index2, diff_index2)
        else:
            same_index, diff_index, edge_index, same_index2, diff_index2 = self._hpg_inputs
...
class ModelSTAGIN(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_classes, num_heads, num_layers, sparsity, dropout=0.5, cls_token='sum', readout='sero', garo_upscale=1.0):
        ...
        self.initial_linear = nn.Linear(112, hidden_dim)
        ...
        for i in range(num_layers):
            ...
            self.transformer_modules.append(ModuleTransformer(112, 2*hidden_dim, num_heads=num_heads, dropout=0.1))
