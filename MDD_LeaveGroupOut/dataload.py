...
class dataloader():
    ...
    def data_split(self, n_folds):
        # Subject-wise K-fold CV with stratification on (diagnosis label + SITE_ID)
        labels = np.array(self.y)
        sites = np.array(self.site)
        # Combine diagnosis and site into a single stratification label
        strat_labels = labels * 1000 + sites
        skf = StratifiedKFold(n_splits=n_folds, shuffle=True, random_state=666)
        cv_splits = list(skf.split(self.raw_features, strat_labels))
        return cv_splits
...
def get_static_affinity_adj(pd_dict, peoples_feature):
    ...
    pd_affinity = create_affinity_graph_from_scores(['SITE_ID', 'AGE_AT_SCAN'], pd_dict)
