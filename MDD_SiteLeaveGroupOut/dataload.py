...
class dataloader():
    ...
    def data_split(self, n_folds):
        # Leave-one-site-out cross-validation based on SITE_ID
        sites = np.array(self.site)
        unique_sites = np.unique(sites)
        cv_splits = []
        for site_id in unique_sites:
            test_ind = np.where(sites == site_id)[0]
            train_ind = np.where(sites != site_id)[0]
            cv_splits.append((train_ind, test_ind))
        return cv_splits
