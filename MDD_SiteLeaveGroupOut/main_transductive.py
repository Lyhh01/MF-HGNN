...
if __name__ == '__main__':
    ...
    # Initializes the data partition for cross-validation (leave-one-site-out).
    ...
    for i in range(len(cv_splits)):
        ...
        # Use the next fold (cyclic) as validation, based on actual number of folds (sites)
        next_i = (1 + i) % n_folds
        val_index.append(cv_splits[next_i][1])
        train_set = np.setdiff1d(cv_splits[i][0], cv_splits[next_i][1])
        train_index.append(train_set)
